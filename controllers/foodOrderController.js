  ...
  try {
    ...
    if (!activeBooking) {
      return res
        .status(403)
        .json({
          message: "You must have an active room booking to order food.",
        });
    }
    ...
    if (!deliveryLocation) {
      return res
        ...
        .json({
          message:
            "Please provide a delivery location (e.g. Room 101 or Table 5)",
        });
    }
...
const getOrders = async (req, res) => {
  try {
    const orders = await FoodOrder.find({ paymentStatus: { $ne: "pending" } })
